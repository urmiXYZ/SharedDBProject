...
namespace MDUA.DataAccess
{
    public partial class UserLoginDataAccess : BaseDataAccess, IUserLoginDataAccess
    {
        ...
        protected void FillObject(UserLoginBase userLoginObject, SqlDataReader reader, int start)
        {
            ...
            if (!reader.IsDBNull(start + 6)) userLoginObject.CreatedBy = reader.GetString(start + 6);
            if (!reader.IsDBNull(start + 7)) userLoginObject.CreatedAt = reader.GetDateTime(start + 7);
            if (!reader.IsDBNull(start + 8)) userLoginObject.UpdatedBy = reader.GetString(start + 8);
            if (!reader.IsDBNull(start + 9)) userLoginObject.UpdatedAt = reader.GetDateTime(start + 9);
